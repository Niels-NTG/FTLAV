...
public class ParseCSV {
	...
	public void writeCSV(String fileName) {
		...
		newRow.put("SHIP TYPE", DataParser.getFullShipType());
		...
		newRow.put("FLEET ADVANCEMENT", Integer.toString(DataParser.getRebelFleetAdvancement()));
		// Log
		...
		newRow.put("SCORE", Integer.toString(DataParser.getCurrentScore()));
		// Encounter
		newRow.put("HAZARDS", DataParser.getBeaconHazards());
		newRow.put("EVENT TEXT", FTLAdventureVisualiser.gameState.getEncounter().getText());
		newRow.put("STORE", DataParser.getStoreListing());
		// Supplies
		...
		newRow.put("CARGO", DataParser.getCargoListing());
		newRow.put("AUGMENTS", DataParser.getAugmentListing());
		newRow.put("OXYGEN LEVEL", Integer.toString(DataParser.getShipOxygenLevel()));
		// Systems
		...
		for (int i = 0; i < FTLAdventureVisualiser.gameState.getTotalCrewHired(); i++) {
			try {
				...
				newRow.put("CREW MEMBER " + (i+1) + " SPECIES", DataParser.getFullCrewType(i));
